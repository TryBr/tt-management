...
const Owners = ({ owners }) => {
  ...
  <Column title="fio" dataIndex="fio" key="fio" className="fio-cell" render={(text, record, index) => <Link to={`${url}/owner/:ownerId`}>{record.fio}</Link>} />
        <Column title="address" dataIndex="address" key="address" />
        <Column title="post" dataIndex="post" key="post" />
        <Column title="menu" dataIndex="menu" key="menu" className="menu-cell" render={() => <Button icon={<OwnerMenuIcon />}></Button>} />
      </Table>
    </Fragment>
  );
};
