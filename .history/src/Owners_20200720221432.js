...
const Owners = ({ owners }) => {
  ...
  return (
    <Fragment>
          ...
          <Col span={8}>
            <div className="select-wrapper">
              ...
              >
                ...
                <Option value="noabc">Не по алфавиту</Option>
              </Select>
            </div>
          </Col>
        </Row>
      ...
      <Table className="owners-table" dataSource={owners} showHeader={false} pagination={false}>
        <Column title="fio" dataIndex="fio" key="fio" className="fio-cell" render={(text, record, index) => <Link to={`${url}/${record.id}`}>{record.fio}</Link>} />
        <Column title="address" dataIndex="address" key="address" />
        <Column title="post" dataIndex="post" key="post" />
        <Column title="menu" dataIndex="menu" key="menu" className="menu-cell" render={(retext, record, indexcord) => <Link to={`${url}/${record.id}/edit`}><Button icon={<OwnerMenuIcon />}></Button></Link>} />
      </Table>
    </Fragment>
  );
};
