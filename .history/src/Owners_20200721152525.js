  ...
  return (
    <Fragment>
      ...
      <Table className="owners-table" dataSource={owners} showHeader={false} pagination={false}>
        <Column title="fio" dataIndex="fio" key="fio" className="fio-cell" render={(text, record, index) => <Link to={`${url}/${index}`}>{record.fio}</Link>} />
        <Column title="address" dataIndex="address" key="address" />
        <Column title="post" dataIndex="post" key="post" />
        <Column title="menu" dataIndex="menu" key="menu" className="menu-cell" render={(text, record, index) => <Link to={`${url}/${index}/edit`}><Button icon={<OwnerMenuIcon />}></Button></Link>} />
      </Table>
    </Fragment>
  );
};
